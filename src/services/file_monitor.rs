use crate::ui::theme_manager::LOADER as THEME_LOADER;
                    ...
                    Ok(Ok(event)) => {
                        ...
                        for path in event.paths {
                            ...
                            if path.starts_with(&attachments_dir) {
                                ...
                                if let Ok(mut loader) = THEME_LOADER.write() {
                                    if let Err(e) = loader.reload_theme_from_file(&path) {
                                        error!("文件监控: 重载主题失败: {e}");
                                    } else {
                        ...
                        if theme_changed_in_batch {
                                ...
                                && now.duration_since(last) < category_debounce
                            {
                                ...
                                let _ = event_tx.send(FileEvent::ThemeChanged);
                            }
                        }
