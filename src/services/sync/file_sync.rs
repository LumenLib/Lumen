...
use sync::{AttachmentBackend, LocalFileManager};
...
pub struct FileSyncService {
    ...
    /// 附件同步状态
    attachment_sync_status: Arc<Mutex<SyncStatus>>,
...
impl FileSyncService {
    pub fn new(
        ...
        let manager = Self {
            ...
            attachment_sync_status: Arc::new(Mutex::new(SyncStatus::Idle)),
            pending_renames,
        };
    ...
    /// 执行附件同步 (供外部手动触发)
    pub fn perform_attachments_sync(
        &self,
        app: Arc<MainApp>,
        ...
        Some(tokio::spawn(async move {
            ...
            let _ = service.sync_local_to_remote_inner(app.clone()).await;
            let _ = service.sync_remote_to_local_inner(app.clone()).await;
            app.notify_ui_changed();
        }))
    }

    /// 阶段 1: 上传本地变更到远程 (Push)
    pub async fn sync_local_to_remote(&self, app: Arc<MainApp>) -> Result<Vec<String>> {
        if !self.backend().await.is_enabled() {
            return Ok(Vec::new());
        }
        self.sync_local_to_remote_inner(app).await
    }

    async fn sync_local_to_remote_inner(&self, app: Arc<MainApp>) -> Result<Vec<String>> {
        // 尝试获取锁
        {
    ...
    /// 强制下载单个文件 (按需下载)
    pub async fn download_single_file(&self, attachment: &models::Attachment) -> Result<bool> {
        let backend = self.backend().await;
    ...
    pub async fn sync_remote_to_local(&self, app: Arc<MainApp>) -> Result<()> {
        let backend = self.backend().await;
        if !backend.is_enabled() {
            return Ok(());
        }
        ...
        drop(backend);
        self.sync_remote_to_local_inner(app).await
    }

    async fn sync_remote_to_local_inner(&self, app: Arc<MainApp>) -> Result<()> {
        // 尝试获取锁
        {
            let mut status = self.attachment_sync_status.lock().await;
            if *status == SyncStatus::Syncing {
    ...
    pub async fn test_backend_config(&self, name: &str, config_json: &str) -> anyhow::Result<()> {
        ...
        let backend = sync::create_backend(name, config_json);
        if !backend.is_enabled() {
