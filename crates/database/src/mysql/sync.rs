...
async fn push_dirty_records(
    ...
    if !lits.is_empty() {
        ...
        for lit in lits {
            ...
            let q = "INSERT INTO literatures (id, title, year, month, day, type, publication_id, volume, issue, pages, abstract_text, doi, arxiv_id, url, notes, keywords, rating, reading_status, is_deleted, version, created_at, updated_at) VALUES (:id, :title, :year, :month, :day, :type, :pub_id, :volume, :issue, :pages, :abstract_text, :doi, :arxiv_id, :url, :notes, :keywords, :rating, :reading_status, :is_deleted, :version, :created_at, NOW()) ON DUPLICATE KEY UPDATE title=VALUES(title), year=VALUES(year), month=VALUES(month), day=VALUES(day), type=VALUES(type), publication_id=VALUES(publication_id), volume=VALUES(volume), issue=VALUES(issue), pages=VALUES(pages), abstract_text=VALUES(abstract_text), doi=VALUES(doi), arxiv_id=VALUES(arxiv_id), url=VALUES(url), notes=VALUES(notes), keywords=VALUES(keywords), rating=VALUES(rating), reading_status=VALUES(reading_status), is_deleted=VALUES(is_deleted), version=VALUES(version), updated_at=NOW()";
            if let Err(e) = conn.exec_drop(q, params! { "id" => &lit.id, "title" => &lit.title, "year" => lit.year, "month" => lit.month, "day" => lit.day, "type" => serde_json::to_string(&lit.literature_type).unwrap_or_default().replace('"', ""), "pub_id" => &publication_id, "volume" => &lit.volume, "issue" => &lit.issue, "pages" => &lit.pages, "abstract_text" => &lit.abstract_text, "doi" => &lit.doi, "arxiv_id" => &lit.arxiv_id, "url" => &lit.url, "notes" => &lit.notes, "keywords" => serde_json::to_string(&lit.keywords).unwrap_or_default(), "rating" => lit.rating, "reading_status" => lit.reading_status.to_string(), "is_deleted" => lit.is_deleted, "version" => lit.version, "created_at" => &lit.created_at }).await {
...
async fn pull_remote_changes(
    ...
    let rows: Vec<mysql_async::Row> = conn.exec("SELECT id, name, publication_type, abbreviation, publisher, ccf_rank, jcr_rank, cas_rank, is_deleted, version, created_at, updated_at FROM publications", ()).await?;
    if !rows.is_empty() {
    ...
    let rows: Vec<mysql_async::Row> = conn.exec("SELECT l.id, l.title, l.year, l.month, l.day, l.type, l.volume, l.issue, l.pages, l.abstract_text, l.doi, l.arxiv_id, l.url, l.notes, l.keywords, l.rating, l.reading_status, l.is_deleted, l.version, l.created_at, l.updated_at, p.id, p.name, p.publication_type, p.abbreviation, p.publisher, p.ccf_rank, p.jcr_rank, p.cas_rank, p.is_deleted, p.version, p.created_at, p.updated_at FROM literatures l LEFT JOIN publications p ON l.publication_id = p.id", ()).await?;
    if !rows.is_empty() {
