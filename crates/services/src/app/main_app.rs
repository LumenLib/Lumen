...
impl MainApp {
    pub fn add_literature(&self, lit: Literature) -> Result<()> {
        ...
        self.op_notify(|| {
            self.literature_service
                .save_literature(self.db.clone(), self.data_changed_notify(), lit)
        })?;
        Ok(())
    }
    ...
    fn delete_literature_inner(&self, id: &str) -> Result<()> {
        ...
        if in_trash {
            ...
            self.literature_service
                .set_literature_folders(&self.db, id, vec!["trash".to_string()])?;
        }
        Ok(())
    }
                    ...
                    match sync.download_single_file(&att).await {
                        Ok(changed) => {
                            if changed {
                                ...
                                if let Some(tx) = &refresh_tx {
                                    let _ = tx
                                        .send(crate::notify::RefreshMsg::DataChanged);
                                }
                                // 同时请求一次后台同步以保持状态一致
                                sync.request_sync();
                            } else {
