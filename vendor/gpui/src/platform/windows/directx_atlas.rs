...
impl DirectXAtlasState {
    fn allocate(
        ...
        let evict_id: Option<AtlasTextureId> = {
            ...
            if texture_kind == AtlasTextureKind::Polychrome {
                ...
                if active >= MAX_POLYCHROME_TEXTURES {
                    ...
                    Some(textures.textures[idx].as_ref().unwrap().id)
                } else {
                    None
                }
            } else {
        ...
        if let Some(tex_id) = evict_id {
            ...
            if size.width.0 as u32 > texture.width || size.height.0 as u32 > texture.height {
                ...
                unsafe {
                    self.device
                        .CreateTexture2D(&texture_desc, None, Some(&mut new_texture))
                        .ok()?;
                }
                texture.texture = new_texture.unwrap();
                texture.width = texture_desc.Width;
                texture.height = texture_desc.Height;
                let mut new_view = None;
                unsafe {
                    self.device
                        .CreateShaderResourceView(&texture.texture, None, Some(&mut new_view))
                        .ok()?;
                }
...
impl DirectXAtlasTexture {
    ...
    fn reset_allocator(&mut self) {
        let size = etagere::Size::new(self.width as i32, self.height as i32);
