    ...
    fn allocate(&mut self, size: Size<DevicePixels>, texture_kind: AtlasTextureKind) -> AtlasTile {
        ...
        if let Some(tex_id) = evict_id {
            ...
            if size.width > texture.size.width || size.height > texture.size.height {
                ...
                let new_size = size.max(&DEFAULT_ATLAS_SIZE);
                let usage = gpu::TextureUsage::COPY | gpu::TextureUsage::RESOURCE;
                texture.raw = self.gpu.create_texture(gpu::TextureDesc {
                    name: "atlas",
                ...
                texture.raw_view = self.gpu.create_texture_view(
                    texture.raw,
                    gpu::TextureViewDesc {
                        name: "",
                        format: texture.format,
